...
async def create_assistant(data, state: FSMContext):
    ...
    keyboard.row(types.InlineKeyboardButton(text='👤 Персональный ассистент', callback_data='personal_assistant'))
    ...
    await message.edit_text(f'Введите токен Вашего бота\n\n'
                            ...
                            f'<b>Персональный ассистент</b> - ассистент, который будет работать только в этом чате и '
...
async def auto_create_assistant_step(message: types.Message, state: FSMContext):
    ...
    if '<START>' in mes and '<END>' in mes:
        ...
        i = await database.add_assistant(message.chat.id, info['token'], info['name'], info['username'],
                                         model=info['model'])
        if not os.path.exists(f"core/assistant/internal_core/static/{i}"):
            os.makedirs(f"core/assistant/internal_core/static/{i}")

        await state.set_state()
        return await assistant_menu(message, SelectAssistant(id=i), state)
...
async def personal_assistant(call: types.CallbackQuery, state: FSMContext):
    ...
    a_id = await database.add_assistant(call.message.chat.id, '', 'Личный ассистент', info.username,
                                        is_personal=True)
